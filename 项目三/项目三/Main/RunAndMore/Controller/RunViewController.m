...
- (void)addWH{
    InputView *view = [[InputView alloc] initWithFrame:CGRectMake(0, 0, 300, 300)];
    [self getTime];
    view.block = ^(NSString *weight,NSString *height){
        User *user = [[User alloc] init];
        user.weight = weight;
        user.height = height;
        user.currentTime = _currentTime;
        [_userArray insertObject:user atIndex:0];
        [_tableView reloadData];
        
        NSDictionary *dic = [[NSDictionary alloc] initWithObjectsAndKeys:weight,@"myWeight",height,@"myHeight",_currentTime,@"time", nil];
        [_saveArray addObject:dic];
        [[NSUserDefaults standardUserDefaults] setObject:_saveArray forKey:@"saveArray"];
